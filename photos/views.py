...
def detail(request, image_id):
    ...
    path3='media/features'
    path4=det_photo.name[:-4]+'.csv'
    feature_image_path=os.path.abspath(os.path.join(BASE_DIR, os.path.join(path3,path4)))
    class_name_=det_photo.class_name

    # feature = run_on_image(imagePath1)

    with open(feature_image_path,'r') as f:
        reader=csv.reader(f)
        for row in reader:
            features1 = [float(x) for x in row[0:]]
        f.close()
            # print row
            # print "********************************************************************************************************"
            # print features1#de el features bta3et el image elly das 3leha bs float

    feature =features1
    searcher = Searcher(path_class)
    results = searcher.search(feature)
    print results
    h_similarity=results[:1]
    print h_similarity
    results=results[1:]
    list_images = results
